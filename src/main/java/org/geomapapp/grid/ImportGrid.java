...
//		***** GMA 1.6.6: Add new filter for GEODAS grids
	...
	private static class ColumnTiler extends ForkJoinTask<Integer> {
		
		private static final long serialVersionUID = 5583009019319028560L; //auto-generated by Eclipse

		private static volatile int colsProcessed = 0;
		
		private int cellsWithData;
		
		private int ix1, ix2, iy1, iy2, ix, x1, x2, xA, xB, y1, y2;
		private int numCols;
		private double mapWrap, offset, scale, wrap, zScale, add_offset;
		private ImportGrid ig;
		private TileIO.Short tileIO;
		private MapProjection mapProj, gridProj;
		private Rectangle bounds;
		private Grid2D grd;
		
		public ColumnTiler(ImportGrid ig_in, TileIO.Short tio_in, MapProjection mapProj_in, MapProjection gridProj_in, Grid2D grd_in,
				int ix1_in, int ix2_in, int iy1_in, int iy2_in, int ix_in, int x1_in, int x2_in, int y1_in, int y2_in,
				double mapWrap_in, double offsetIn, double scaleIn, double wrapIn, double zScaleIn, double addOffsetIn) {
			ig = ig_in;
			tileIO = tio_in;
			mapProj = mapProj_in;
			gridProj = gridProj_in;
			grd = grd_in;
			bounds = grd.getBounds();
			ix1 = ix1_in;
			ix2 = ix2_in;
			iy1 = iy1_in;
			iy2 = iy2_in;
			...
			x1 = x1_in;
			x2 = x2_in;
			xA = (int)Math.max(ix*320, x1);
			xB = (int)Math.min((ix+1)*320, x2);
			mapWrap = mapWrap_in;
			while (xA >= mapWrap) {
				xA -= mapWrap;
				xB -= mapWrap;
...
//			mapWrap = 360 * ((PolarStereo) mapProj).getScale();
		...
		for( int ix=ix1 ; ix<=ix2 ; ix++) {
			 ...
			 *	double mapWrap_in, double offsetIn, double scaleIn, double wrapIn, double zScaleIn, double addOffsetIn)
			 */
			ColumnTiler ct = new ColumnTiler(this, tileIO, mapProj, gridProj, grd, ix1, ix2, iy1, iy2, ix, x1, x2, y1, y2, mapWrap, offset, scale, wrap, zScale, add_offset);
			fjp.submit(ct);
//			for( int iy=iy1 ; iy<=iy2 ; iy++) {
