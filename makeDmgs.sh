...
then
    echo "You want to package GMA version $1 as a dmg."
	...
	if [ -z "${gma_loc}" ]
	then
	    echo "Could not find GeoMapApp.app anywhere in ${root_dir}."
	else
	    executable=$(find ${gma_loc} -name "GeoMapApp")
	    src_dir=$(dirname ${gma_loc})
	    echo "Found GeoMapApp.app in ${src_dir}."
	    if [[ -z "${executable}" ]]
	    then
	       echo "Could not find the executable within ${gma_loc}. Skipping."
	    else
